<template>
    <div>
        ...
        <div class="panel">
            <div class="header">
                افزون تنوع قیمت جدید برای {{ title }}
            </div>
                ...
                <div v-if="account_status=='active'">
                   ...
                   <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input v-model="send_time" :options="options" type="number"  class="form-control left price_input">
                        <span class="has_error" v-if="send_time_error">{{ send_time_error }}</span>
                    </div>
                    

                    <button class="btn btn-success"  v-on:click="add()">ثبت تنوع قیمت</button>
                </div>
                <div v-else>
...
export default {
        ...
        return {
            ...
            BreadcrumbList:[
               { path:'/products/warranty/'+this.$route.params.id,title:'مدیریت تنوع های قیمت' },
                { path:'/products/warranty/add/'+this.$route.params.id,title:'افزودن تنوع قیمت جدید' },
            ],
            options: {
                delimiter: ',',
                numeral: true,
    ...
    methods:{
        add:function()
        {
            ...
            if(check_warranty && check_price1 && check_price2 && check_product_number && check_product_number_cart && check_send_time) 
            {
                if(this.send_form)
                {
                   ...
                   formData.append('warranty_id',this.warranty_id);
                   ...
                   this.axios.post(url,formData,this.setHeader()).then(response=>{
                       ...
                       if(result)
                       {
                          if(response.data=='ok')
                          {
                                this.$router.push({name:'ProductWarranty',params:{secusss:'ok','id':this.$route.params.id}})
                          }
        ...
        getColorOptionContent: function (color_code, color_name) {
            const background = 'background:#' + color_code;
            return "<span class='color_option' style='" + background + "'>" + color_name + '</span>';
        },
    }
}
</script>
