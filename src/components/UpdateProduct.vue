        ...
        <div class="panel">
             ...
             <div class="panel_content">
                 ...
                 <div v-if="account_status=='active'">
                    ...
                    <div class="row">
                       <div class="col-md-6">
                           <div class="form-group">
                            ...
                            <div class="form-group">
                                <button class="btn btn-primary" style="margin-top:20px;"  v-on:click="updateProduct()">ویرایش محصول</button>
                            </div>
                       </div>
                       <div class="col-md-6">
                    ...
                    <div class="alert alert-danger reject_message" v-for="message in reject_message" :key="message.id">
                        <div>
                            <span>ثبت شده توسط کارشناس دیجی آنلاین</span>
                            <span> در تاریخ </span>
                            <span>{{ getDate(message.time) }}</span>
                        </div>
...
export default {
    components:{VueEditor,AccountStatus,ServerErrorMessage},
    mixins:[myMixin],
     data(){
        return{
            server_error:false,
            BreadcrumbList:[
                ...
                {path:'/products/'+this.$route.params.id+'/edit',title:'ویرایش محصول'}
            ],
            catList:[],
            brandList:[],
            colorList:[],
    ...
    methods:{
        ...
        updateProduct:function(){
            if(this.send_form)
            {
                ...
                if(validateTitle && validateContent && validateCatId && validateBrandId && validateImage)
                {
                    ...
                    const url=this.$siteUrl+'/api/seller/product/'+this.product_id;
                    ...
                    this.axios.post(url,formData,this.setHeader(true)).then(response=>{
                        ...
                        if(result)
                        {
                            if(response.data=="ok")
                            {
                                this.$router.push({name:'ProductList',params:{success:'ok',message:'ویرایش محصول با موفقیت انجام شد'}})
                            }
        ...
        {
            this.showLoading=true;
            const url=this.$siteUrl+'/api/seller/product/'+this.product_id+'/edit';
            this.axios.get(url,this.setHeader()).then(response=>{
                this.showLoading=false;
                const result=this.setAccountStatus(response.data);
                if(result)
                {
                    if(response.data.product.title!=undefined)
                    {
                        this.title=response.data.product.title;
                        this.catId=response.data.product.cat_id;
                        this.brandId=response.data.product.brand_id;
                        this.content=response.data.product.tozihat;
                        this.image_src=this.$siteUrl+'/files/thumbnails/'+response.data.product.image_url;
                        this.ename=response.data.product.ename;
                        if(response.data.product.get_product_color.length>0)
                        {
                            for(let i=0;i<response.data.product.get_product_color.length;i++)
                            {
                                this.colorId[i]=response.data.product.get_product_color[i].color_id;
                            }
                        }
                        this.reject_message=response.data.reject_message;
